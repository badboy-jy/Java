...
public class NewsOutServlet extends HttpServlet {
	@Override
	...
	protected void doPost(HttpServletRequest request,
		...
		if (name == null || name.equals("") || news == null || news.equals("")) {
	 ...
	 if (!dbnews.insertDataToDB(name, news)) {
			//注册成功
			data.setCode(0);
			data.setMsg("发布成功!!");
			ResultSet rs = dbnews.getUser();
			int id = -1;
			if (rs != null) {
				try {
					while (rs.next()) {
						if (rs.getString("news_name").equals(name)
								&& rs.getString("news_news").equals(news)) {
