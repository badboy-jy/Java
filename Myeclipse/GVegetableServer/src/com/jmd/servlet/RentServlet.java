...
public class RentServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		DBLandRent dblandrent = new DBLandRent();
		dblandrent.openConnect();
		// 打开数据库连接
		BaseBean data = new BaseBean(); // 基类对象，回传给客户端的json对象
		UserBean userBean = new UserBean(); //user的对象
		if (dblandrent.isExistInDB(name, phone)) {
			// 判断账号是否存在
			data.setCode(-1);
			data.setData(userBean);
			data.setMsg("该信息已存在");
		} else if (!dblandrent.insertDataToDB(name, shuliang,price,phone,weixin,year)) {
			//注册成功
			data.setCode(0);
			data.setMsg("发布成功!!");
			ResultSet rs = dblandrent.getUser();
			int id = -1;
			if (rs != null) {
				try {
					while (rs.next()) {
						if (rs.getString("landrent_name").equals(name)
								&& rs.getString("landrent_phone").equals(phone)) {
							id = rs.getInt("landrent_id");
						}
					}
					userBean.setId(id);
		...
		dblandrent.closeConnect(); //关闭数据库连接
	}
}
