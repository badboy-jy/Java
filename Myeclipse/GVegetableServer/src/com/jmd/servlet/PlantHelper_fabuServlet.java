...
public class PlantHelper_fabuServlet extends HttpServlet {
	@Override
	...
	protected void doPost(HttpServletRequest request,
		...
		DBPlantHelper dbplanthelper = new DBPlantHelper();
		dbplanthelper.openConnect();
		// 打开数据库连接
		BaseBean data = new BaseBean(); // 基类对象，回传给客户端的json对象
		UserBean userBean = new UserBean(); //user的对象
		if (dbplanthelper.isExistInDB(name, phone)) {
			// 判断账号是否存在
			data.setCode(-1);
			data.setData(userBean);
			data.setMsg("该信息已存在");
		} else if (!dbplanthelper.insertDataToDB(name, renshu,price,phone,weixin,time)) {
			//注册成功
			data.setCode(0);
			data.setMsg("发布成功!!");
			ResultSet rs = dbplanthelper.getUser();
			int id = -1;
			if (rs != null) {
				try {
					while (rs.next()) {
						if (rs.getString("planthelper_name").equals(name)
								&& rs.getString("planthelper_phone").equals(phone)) {
							id = rs.getInt("planthelper_id");
						}
					}
					userBean.setId(id);
		...
		dbplanthelper.closeConnect(); //关闭数据库连接
	}
}
