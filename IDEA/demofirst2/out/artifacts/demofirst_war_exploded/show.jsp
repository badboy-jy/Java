...
<head>
    ...
    <script type="text/javascript">
        ...
        function query(){
            ...
            location.href="/entry?method=getall&query1="+text;//获得类型列表的请求
        }
        //分类查询的onchange事件
        function query2(){
            ...
            location.href="/entry?method=getall&query2="+text;//获得类型列表的请求
        }
        //回显
       ...
       <%-- 分页的各种按钮链接--%>
        <tr>
            ...
            <td colspan="7" align="right">
                <a href="/entry?method=getall&index=1&&query1=<%=request.getAttribute("query1")%>&&query2=<%=request.getAttribute("query2")%>">首页</a>
                <a href="/entry?method=getall&index=<%=pageindex==1?1:pageindex-1%>&&query1=<%=request.getAttribute("query1")%>&&query2=<%=request.getAttribute("query2")%>">上一页</a>
                <% for (int i=1;i<=pages;i++) {%>
                <%if (i==pageindex){%>
                <a href="/entry?method=getall&index=<%=i%>&&query1=<%=request.getAttribute("query1")%>&&query2=<%=request.getAttribute("query2")%>"><%=i%></a>
                <%}else{%>
                <a href="/entry?method=getall&index=<%=i%>&&query1=<%=request.getAttribute("query1")%>&&query2=<%=request.getAttribute("query2")%>" style="text-decoration: none"><%=i%></a>
                <%}%>
                <%}%>

                <a href="/entry?method=getall&index=<%=pageindex==pages?pages:pageindex+1%>&&query1=<%=request.getAttribute("query1")%>&&query2=<%=request.getAttribute("query2")%>">下一页</a>
                <a href="/entry?method=getall&index=<%=pages%>&&query1=<%=request.getAttribute("query1")%>&&query2=<%=request.getAttribute("query2")%>">尾页</a>
